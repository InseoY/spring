...
<link rel="stylesheet" type="text/css" media="screen" href="http://ui.clickmedi.co.kr/1016/ui1016.css?191231092508">
...
$(document).ready(function() {
	...
	if('list' != 'view' || 'boards' == 'babyphotos'){
		$("meta[property='twitter:title']").each(function(index, ele){
			if(index == 0){
				// 트위터 title
				//$("meta[property='twitter:title']").attr("content", "["+'IS NursHos'+"] " + '공지사항');
				$(this).attr("content", "["+'IS NursHos'+"] " + '공지사항');
			}
		})
...
<meta property="og:url" content="http://www.kn-hospital.com/boards/list/1292" />
...
$(document).ready(function() {
	...
	if('' != '13873'){
		$("#sub_title").text("커뮤니티");
		$("#sub_menu").empty();
		$("#sub_menu").html('<ul class="skin_menu"><li class="on"><a href="/ISNursHos/community/notice"  ><span>공지사항</span></a><li><a href="/ISNursHos/community/counsel"  ><span>입원상담</span></a></li></ul>');
		$("#content_location").html('<h1 id="content_title">공지사항</h1><div id="center_contents" class="tit_location"><a href="/ISNursHos/">HOME</a> &gt; 커뮤니티 &gt; 공지사항');
	}else{
		$("#sub_title").text('');
		$("#sub_menu").empty();
		$("#sub_menu").html('');
...
<!-- new widget -->
<!-- 추가 페이지 및 뷰 페이지 form -->
<form id="boardEditor" action="/boards/list/1292" method="GET">
</form>
<!-- 리스트 검색 form -->
<form id="boardList" action="/boards/list/{boardMngtNbr}" method="GET">
<input id="hpNbr" name="hpNbr" type="hidden" value="1016"/>
<input id="boardMngtNbr" name="boardMngtNbr" type="hidden" value="1292"/>
	...
	<table class="board_list_type1 common">
		...
		<tbody>
			...
			<!-- 공개 글 -->
				...
				<tr onclick="javascript:boardView('38895');" class="notice_cell">
...
박*O
				...
				<td class="0115005" style="display: none;">2018-11-20</td>
				<td class="0115003" style="display: none;">111</td>
			</tr>
			...
			<!-- 공개 글 -->
				...
				<tr onclick="javascript:boardView('38895');" style="cursor: pointer;">
